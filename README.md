...
```mermaid
graph TD
    User -->|Text / Voice Input| AndroidUI[Android UI (Jetpack Compose)]
    AndroidUI -->|API Request| AIService[AI Service Layer (Gemini API)]
    AIService -->|AI Response| ViewModel[ViewModel → UI State]
    ViewModel -->|Optional Storage| Storage[Room / Firestore]
```

*(Note: If Mermaid diagrams are not supported in your viewer, see the text representation below)*
